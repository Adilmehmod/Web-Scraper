...
(async () => {
    const nameSheet = 'swiss.csv';
    const googleUrl = 'https://www.google.com/maps/search/uni+and+colleges+New+Zealand/@-40.0836943,171.3124923,7z?entry=ttu';
    ...
    const scrapePageData = async (pageUrl) => {
        ...
        try {
            await newPage.goto(pageUrl, { timeout: 120000 }); // Increase the timeout to 60 seconds
        } catch (error) {
        ...
        // Extract data if the elements exist
        ...
        if (nameElement) {
            name = await newPage.evaluate(element => element.textContent, nameElement);
            name = `"${name}"`;
        }
        ...
        if (ratingElement) {
            rating = await newPage.evaluate(element => element.textContent, ratingElement);
            rating = `"${rating}"`;
        }
        ...
        if (reviewsElement) {
            reviews = await newPage.evaluate(element => element.textContent.replace(/\(|\)/g, ''), reviewsElement);
            reviews = `"${reviews}"`;
        }
        ...
        if (categoryElement) {
            category = await newPage.evaluate(element => element.textContent, categoryElement);
            category = `"${category}"`;
        }

        const addressElement = await newPage.$('button[data-tooltip="Copy address"]');
        if (addressElement) {
            address = await newPage.evaluate(element => element.textContent, addressElement);
            address = `"${address}"`;
        }

        const websiteElement = await newPage.$('a[data-tooltip="Open website"]') || await newPage.$('a[data-tooltip="Open menu link"]');
        if (websiteElement) {
            website = await newPage.evaluate(element => element.getAttribute('href'), websiteElement);
            website = `"${website}"`;
        }

        const phoneElement = await newPage.$('button[data-tooltip="Copy phone number"]');
        if (phoneElement) {
            phone = await newPage.evaluate(element => element.textContent, phoneElement);
            phone = `"${phone}"`;
        }
    ...
    if (fs.existsSync(nameSheet)) {
        // Read existing data
        const existingData = fs.readFileSync(nameSheet, 'utf8');
        // Append new data to existing data
        fs.writeFileSync(nameSheet, existingData + csvRows);
    } else {
